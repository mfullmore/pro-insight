...
- `vendor.js` mechanically copies `xlsx.core.min.js` and `convert-core.js` verbatim (`fs.copyFileSync`, no transformation), and wraps `app.css`, `hyperformula.bundle.js`, and `render-core.js` as TypeScript string constants (`appStyles.ts`, `hyperformulaSource.ts`, `renderCoreSource.ts`) via a straight `JSON.stringify` — nothing here is hand-edited or hand-ported. (`hyperformula.bundle.js`/`render-core.js` moved to string-embedding after a real bug — see "Formulas render fine but every value shows #ERROR!" below.)
...
Verified as of this writing: every file in `src/webparts/interactivePlan/vendor/` reproduces its `browser-converter/core/` source byte-for-byte (the two `.ts` string constants included — `JSON.stringify` round-trips exactly). The only real risk here is forgetting to **re-run `node vendor.js` and rebuild** after a `browser-converter/core/` change — a staleness risk, not a drift-while-editing risk — already covered under "Rebuilding after changing shared code" above.
...
### Formulas render fine but every value shows #ERROR! — not just the one genuinely-unsupported formula

This was a real, previously-undetected bug (fixed as of this writing): live formula recalculation likely never worked at all on this web part, despite "recalculates live" being reported as confirmed in earlier testing — that testing apparently only ever validated that raw numbers were editable and round-tripped through Save as new version, never that a computed formula's *value* was actually correct.

**Root cause**: tsc's `allowJs` pipeline (`target: es5` — see `tsconfig.json`) silently corrupted `hyperformula.bundle.js`'s evaluator when it recompiled the already-built bundle. Parsing still worked fine (`getCellFormula`/`getCellType`/`doesCellHaveFormula` all reported correctly) but `getCellValue` threw `"Value of the formula cell is not computed."` for *every* formula, even a trivial `=A1+B1` in a brand-new engine with zero dependencies — reproduced in complete isolation, entirely outside of SPFx/webpack/SharePoint, by extracting the exact tsc-transformed module and re-running it standalone. `render-core.js` had to move with it: its outer IIFE captures `HyperFormula` from `root.HyperFormula` exactly once, at the moment it executes, so a runtime-loaded HyperFormula (see below) arriving after that capture would leave `createEngine` closed over `undefined`.

**Fix**: both now ship as inert TS string constants (`vendor.js`, see "Vendor drift check" above) and get executed as real, untransformed JS at runtime via `ensureEngineLibsLoaded()` in `InteractivePlanWebPart.ts`, instead of being tsc-compiled. **If you're touching that function**: the first version of this fix injected the source via a `<script>` element with `.textContent` set — this compiles and deploys cleanly, and *looks* like it should work, but silently does nothing on any tenant whose CSP doesn't include `'unsafe-inline'` for `script-src` (confirmed on this project's test tenant: the `<script>` gets appended, no exception is thrown, no console error appears, and `InteractivePlanRender`/`HyperFormula` simply never get defined — check `window.trustedTypes` and try appending a trivial test script as your first diagnostic step if you see `InteractivePlanRender is not defined`). `new Function(source)()` was confirmed to still be permitted (this tenant's CSP allows `'unsafe-eval'`) and produces identical results, since `window.HyperFormula = HyperFormula;`/`root.InteractivePlanRender = ...` inside the executed source still set real globals regardless of the enclosing function's scope.

**How to tell if you've regressed this**: open any sheet with a real cross-sheet or arithmetic formula and check the *value*, not just that the cell renders — e.g. `budget-test-3.xlsx`'s Engineering sheet, Overhead Rate column, should show `0.15`, not `#ERROR!`.
